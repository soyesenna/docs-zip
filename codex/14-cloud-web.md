...
| 시크릿 (Secrets) | 추가 암호화 레이어 | 설정 스크립트만 | 에이전트 단계 시작 전 제거됨 |

> **중요**: 시크릿은 보안상 에이전트 단계가 시작되기 전에 제거됩니다. 에이전트가 시크릿 값에 접근할 수 없습니다.
...
기본적으로 Codex는 **에이전트 단계에서 인터넷 접근을 차단**합니다. 설정 스크립트는 인터넷 접근이 허용되어 의존성 설치가 가능합니다.
...
Codex가 반응(👀)하고 리뷰를 게시합니다. GitHub에서 P0(심각) 및 P1(높음) 이슈만 표시하여 리뷰 코멘트가 우선순위가 높은 위험에 집중되도록 합니다.
...
> **Preview**: Sites는 현재 **ChatGPT Business 및 Enterprise** 워크스페이스에서 사용 가능합니다. 추가 플랜은 순차적으로 지원됩니다.

### 시작하기

#### Enterprise 워크스페이지에서 활성화

1. 워크스페이스 관리자가 ChatGPT 관리 설정에서 RBAC 제어 열기
2. Sites를 적절한 역할에 대해 활성화
3. Business 워크스페이스는 기본 활성화되어 있어 이 단계 생략 가능
...
플러그인을 명시적으로 지정하려면 `@Sites`를 사용합니다.

#### 저장 vs 배포

| 단계 | 설명 |
|---|---|
| **버전 저장 (Save)** | 배포 가능한 사이트 빌드 + 소스 Git 커밋과 연결. 리뷰용 후보 |
| **버전 배포 (Deploy)** | 저장된 버전을 프로덕션에 게시. 프로덕션 URL 반환 |

> 모든 Sites 배포 URL은 프로덕션 배포입니다. 빌드를 라이브로 만들기 전에 검토하려면 배포 없이 버전을 저장하세요.

### `.openai/hosting.json` 구조

Sites 프로젝트는 로컬 소스 프로젝트를 Sites 호스팅과 연결합니다. 연결 정보는 `.openai/hosting.json`에 저장됩니다.
...
- 환경 값을 변경한 후에는 승인된 저장 버전을 재배포하세요
...
| **시크릿 관리** | `.env`, 로컬 파일 | 환경 변수 + 암호화된 시크릿 (에이전트 접근 불가) |
