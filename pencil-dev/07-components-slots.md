...
모든 표준 디자인 요소 — 프레임, 도형, 텍스트 — 를 재사용 가능한 컴포넌트로 변환할 수 있습니다.

### 생성 방법

| 방법 | 설명 |
| --- | --- |
| **단축키** | 요소를 선택한 후 `Cmd/Ctrl + Option/Alt + K` |
| **속성 패널 버튼** | 속성 패널 상단의 **"Create component"** 버튼 클릭 |

### 시각적 구분

| 요소 | 경계 상자 색상 | 설명 |
| --- | --- | --- |
| **컴포넌트 원본** | **마젠타(Magenta)** | 컴포넌트 원본은 선택 시 마젠타 경계 상자로 표시 |
| **인스턴스** | **바이올렛(Violet)** | 인스턴스는 선택 시 바이올렛 경계 상자로 표시 |
...
캔버스에서 컴포넌트 원본을 복사하면 인스턴스가 생성됩니다.

### 원본 탐색

속성 패널의 **"Go to component"** 버튼을 클릭하면 인스턴스의 원본 컴포넌트로 빠르게 이동할 수 있습니다.
...
슬롯은 컴포넌트 내에서 요소를 드롭할 수 있는 지정된 영역입니다. 컴포넌트에 유연하고 커스터마이징 가능한 영역을 정의할 수 있습니다.

### 슬롯 생성 (GUI)

| 단계 | 설명 |
| --- | --- |
| 1 | 프레임을 생성하고 컴포넌트로 변환 (`Cmd/Ctrl + Option/Alt + K` 또는 속성 패널의 **"Create component"** 버튼) |
| 2 | 원하는 대로 스타일링 |
| 3 | 속성 패널 상단의 **"Make a slot"** 버튼 클릭 |

| 규칙 | 설명 |
| --- | --- |
| 빈 프레임만 가능 | 컴포넌트 원본의 **빈 프레임(empty frames)** 만 슬롯으로 변환할 수 있음 |
| 대각선 표시 | 슬롯은 캔버스에서 **대각선(diagonal lines)** 으로 표시되어 요소를 드롭할 수 있는 영역임을 나타냄 |

### 추천 슬롯 컴포넌트 (Suggested Slot Components)

같은 `.pen` 파일 내의 다른 컴포넌트를 "추천 슬롯 컴포넌트"로 지정할 수 있습니다. 예를 들어 `table` 컴포넌트의 슬롯에 `table-row`를 추천 콘텐츠로 지정할 수 있습니다. 추천 슬롯 컴포넌트는 인간 및 AI 디자이너 모두에게 해당 슬롯에 어떤 요소가 들어가야 하는지 안내합니다.

| 단계 | 설명 |
| --- | --- |
| 1 | 컴포넌트 원본에서 슬롯이 있는 레이어 선택 |
| 2 | 속성 패널 상단의 **"Slots"** 행에서 **`+`** 버튼 클릭 |
| 3 | 추천으로 지정할 컴포넌트 선택 |

### 슬롯에 요소 삽입 (GUI)

컴포넌트 인스턴스를 생성한 후 슬롯에 요소를 채울 수 있습니다:

| 방법 | 설명 |
| --- | --- |
| **드래그 앤 드롭** | 요소를 슬롯 영역으로 직접 드래그 |
| **복사-붙여넣기** | 요소를 복사한 후 슬롯에 붙여넣기 |
