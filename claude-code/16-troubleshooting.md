...
먼저 `claude --safe-mode`로 시작하세요. 이 명령은 `CLAUDE.md`, 스킬, 플러그인, 훅, MCP 서버, 커스텀 명령 및 에이전트를 포함한 모든 커스터마이징을 비활성화한 세션을 시작합니다. 인증, 모델 선택, 내장 도구, 권한은 정상적으로 작동합니다. safe mode에서 문제가 사라지면 커스터마이징 중 하나가 원인이므로, 위의 대상화된 점검으로 어떤 항목인지 특정하세요. 조직이 배포한 managed 설정은 safe mode에서도 부분적으로 적용되므로, 정책으로 구성된 훅과 상태 표시줄은 safe mode에서도 실행됩니다.

대상화된 점검으로 원인을 찾을 수 없거나 설정 자체가 의심되는 경우, 일반 설정을 로드하지 않는 세션과 비교하세요. `CLAUDE_CONFIG_DIR`을 빈 디렉토리로 지정하여 `~/.claude`의 모든 항목을 우회하고, `.claude` 폴더, `.mcp.json`, `CLAUDE.md`가 없는 디렉토리에서 실행하여 프로젝트 구성도 건너뛰세요.
...
4. `claude --safe-mode`로 재시작하여 플러그인, MCP 서버, 훅 중 원인인지 확인. 이 세션에서는 모든 커스터마이징이 비활성화되며, 사용량이 떨어지면 [클린 설정으로 테스트](#클린-설정으로-테스트)에서 어떤 항목이 원인인지 특정
...
Claude Code는 처리할 수 없는 이미지를 텍스트 플레이스홀더로 교체하고 재시도하므로, 후속 메시지는 정상적으로 성공합니다. 단, v2.1.142 이전 버전에서는 붙여넣은 이미지가 대화에 남아 매 후속 메시지마다 같은 에러가 반복될 수 있었습니다. 이 구버전에서 복구하려면 Esc를 두 번 눌러 이미지가 추가된 턴 이전으로 이동하세요.

**해결 방법:**

- 붙여넣기 전에 이미지 리사이즈. API는 단일 이미지 최대 8000px (긴 변), 다수 이미지 시 2000px 허용
...
에러가 표시되지 않는데 응답 품질이 기대보다 낮으면, 원인은 보통 모델 자체보다 대화 상태입니다. Claude Code는 자동으로 모델 버전을 변경하지 않습니다. 단, 세 가지 특정한 경우에는 폴백(fallback) 모델로 전환할 수 있습니다:

- 설정된 `--fallback-model`이 가용성 에러 후 해당 턴에만 인계받으며, 트랜스크립트에 알림이 표시됨
- Bedrock 또는 Vertex AI 시작 검사가 기본 모델을 사용할 수 없는 것으로 확인됨
- Fable 5에서 자동 모델 폴백이 세션을 기본 Opus 모델로 이동하고 트랜스크립트에 알림을 표시함

아래의 모델 선택 확인이 두 번째와 세 번째 경우를 감지하며, 첫 번째는 `/model` 변경이 아닌 트랜스크립트 알림으로 나타납니다. Model configuration에서 각 폴백 적용 시점을 설명합니다.
